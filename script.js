window.onload = () => {
    // ========== الكود الأصلي (مع التعديلات المطلوبة) ==========
    ...
    // ======== [إضافة] دوال جديدة للتحكم في زر الطول وإظهار chat-div ========
    function updateTxtBtnForMessage(wrapper) {
        const message = wrapper.querySelector('.message.bot');
        const txtBtn = wrapper.querySelector('.txt-btn');
        if (!message || !txtBtn) return;
        
        const fullText = message.textContent || message.innerText;
        ...
        if (textLength > LONG_MESSAGE_THRESHOLD || numberOfLines >= LONG_MESSAGE_LINES_THRESHOLD) {
            ...
            // تطبيق الوضع المختصر (Short) افتراضياً
            applyDefaultShortState(message, txtBtn);
        } else {
            txtBtn.style.display = 'none';
            // إزالة الحالة المختصرة إن وجدت
            message.classList.remove('collapsed');
            message.style.maxHeight = '';
        }
    }

    function forceShowChatDivs() {
        document.querySelectorAll('.bot-message-wrapper .chat-div.Wave-all:not(.show)').forEach(chatDiv => {
            chatDiv.classList.add('show');
            const wrapper = chatDiv.closest('.bot-message-wrapper');
            if (wrapper) updateTxtBtnForMessage(wrapper);
        });
    }
    ...
    function loadMessages() {
        ...
        if (saved) {
            ...
            setTimeout(() => {
                    ...
                    if (wrapper) {
                        ...
                        suggestionButtons.forEach(button => {
                            ...
                            newButton.addEventListener('click', (e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                // [تعديل] إظهار chat-div للرسائل السابقة قبل معالجة الاقتراح الجديد
                                forceShowChatDivs();
                ...
                // [إضافة] تحديث زر الطول لجميع الرسائل المحفوظة بعد التحميل
                document.querySelectorAll('.bot-message-wrapper').forEach(wrapper => {
                    updateTxtBtnForMessage(wrapper);
                });
            ...
            if (currentIndex < text.length) {
                ...
                if (!isWelcomeMessage && chatDiv) {
                    if (!hasMoreParts) {
                        ...
                        // [إضافة] تحديث زر الطول بعد اكتمال الكتابة
                        updateTxtBtnForMessage(wrapper);
                    }
                    ...
                    currentMessage = addBotMessageWithTyping(chunks[currentIndex], true, isFirstChunk, () => {
                        ...
                        if (currentIndex < chunks.length) {
                            ...
                            if (currentWrapper) {
                                ...
                                // [إضافة] تحديث زر الطول بعد اكتمال آخر جزء
                                updateTxtBtnForMessage(currentWrapper);
                            }
                            saveMessages();
                        }
                    }, hasMoreParts);
    ...
    function handleQuestion(itemOrText) {
        // [إضافة] إظهار chat-div لأي رسائل بوت سابقة غير مكتملة
        forceShowChatDivs();
    ...
    searchInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === "ArrowDown") {
            ...
            if (searchInput.value.trim()) {
                // [تعديل] إظهار chat-div للرسائل السابقة قبل معالجة السؤال الجديد
                forceShowChatDivs();
    ...
    function initNoteOverlay() {
        let overlayNote = document.getElementById('overlay-note');
